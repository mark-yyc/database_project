...
SET month_num=6;
WHILE month_num<7 DO-- 只有6月的接诊计划

SET day_num=1;
WHILE day_num<32 DO  -- 每一个月是周一到周五

SET cur_date='';
IF day_num<=5 OR (day_num>=8 AND day_num<=12)OR(day_num>=15 AND day_num<=19)OR(day_num>=22 AND day_num<=26) THEN
